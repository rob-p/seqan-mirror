...
def getCategories(tree):
    categories = set()
    for entryTypeNode in tree.root.children.itervalues():
        for entryNode in entryTypeNode.children.itervalues():
            for node in entryNode.children.itervalues():
                if node.key == 'cat':
    ...
    for entryTypeNode in tree.root.children.itervalues():
        for entryNode in entryTypeNode.children.itervalues():
            for node in entryNode.children.itervalues():
                if node.key == 'cat':
                    for text in node.texts:
                        result.setdefault(text, []).append(node)
    return result
...
class TplDocsCreator(object):
    ...
    def createAllCategoriesPage(self):
        """Create the "all categories" page for the top left frame."""
        print >>sys.stderr, 'Creating "All Categories" Page'
        # Collect all categories.
        categories = getCategories(self.tree)
        # Build page.
        srcPath = os.path.join(self.tpl_path, 'all_categories.html')
        destPath = os.path.join(self.out_path, 'all_categories.html')
        with open(destPath, 'wb') as f:
            template = pyratemp.Template(filename=srcPath)
            res = template(title='All Categories',
                           tree=self.tree,
                           categories=categories,
                           now=datetime.datetime.utcnow(),
                           time=time,  # for now.strftime()
                           html=HtmlHelper(self.error_logger, self.tree, os.path.dirname(destPath)),
                           core=core)
            f.write(res.encode('utf-8'))

    def createCategoriesOverviewPages(self):
        print >>sys.stderr, 'Create Category Overview Pages'
        nodes_by_category = getNodesByCategory(self.tree)
        # Write out file for all categories:
        srcPath = os.path.join(self.tpl_path, 'category_entities.html')
        destPath = os.path.join(self.out_path, 'all_entities.html')
        with open(destPath, 'wb') as f:
            nodes = []
            for cat_nodes in nodes_by_category.itervalues():
                nodes += cat_nodes
            template = pyratemp.Template(filename=srcPath)
            res = template(title='All Entities - SeqAn Library',
                           nodes=nodes,
                           now=datetime.datetime.utcnow(),
                           time=time,  # for now.strftime()
                           html=HtmlHelper(self.error_logger, self.tree, os.path.dirname(destPath)),
                           core=core)
            f.write(res.encode('utf-8'))
        # Write out one file for each category.
        for category, nodes in nodes_by_category.iteritems():
            srcPath = os.path.join(self.tpl_path, 'category_entities.html')
            destPath = os.path.join(self.out_path, category, 'category_entities.html')
            if not os.path.exists(os.path.dirname(destPath)):
                os.makedirs(os.path.dirname(destPath))
            with open(destPath, 'wb') as f:
                template = pyratemp.Template(filename=srcPath)
                res = template(title='Category - ' + category +' - SeqAn Library',
                               nodes=nodes,
                               now=datetime.datetime.utcnow(),
                               time=time,  # for now.strftime()
                               html=HtmlHelper(self.error_logger, self.tree, os.path.dirname(destPath)),
...
def createDocs(error_logger, tree, tpl_path, out_path):
    ...
    creator1.createAllCategoriesPage()
    creator1.createCategoriesOverviewPages()
